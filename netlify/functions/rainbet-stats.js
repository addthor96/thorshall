exports.handler = async function () {
  ...
  ["wager","ngr","deposits_sum","first_deposits_count","visits_count","registrations_count"]
    .forEach(c => params.append("columns[]", c));

  params.append("group_by[]", "day");

  const url = `https://portal.rainbetpartners.com/api/customer/v1/partner/report?${params}`;
  ...
  try {
    ...
    const totals = data?.rows?.totals?.data || [];

    const get = (name) => {
      const item = totals.find(x => x.name === name);
      if (!item) return 0;
      if (typeof item.value === "object") return Number(item.value.amount || 0);
