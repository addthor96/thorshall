...
async function fetchRadhikaReport(token, range) {
  ...
  params.append("campaign_ids[]", RADHIKA_CAMPAIGN_ID);
...
exports.handler = async function (event) {
  ...
  try {
    const metrics = await fetchRadhikaReport(token, range);
    const billingMetrics = range.key === "30d"
      ? metrics
      : await fetchRadhikaReport(token, billingRange);
    ...
    const payload = {
      ...
      from: range.from,
      to: range.to,
      campaignId: RADHIKA_CAMPAIGN_ID,
      partners: {
        radhika: {
          name: "Radhika",
          connected: true,
