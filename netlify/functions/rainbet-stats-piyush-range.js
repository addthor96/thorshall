...
const PIYUSH_CAMPAIGN_ID = "81234";
...
async function fetchPiyushReport(token, range) {
  ...
  params.append("campaign_ids[]", PIYUSH_CAMPAIGN_ID);
...
exports.handler = async function (event) {
  if (!isAuthorized(event, "piyush")) return unauthorized();
  ...
  try {
    const metrics = await fetchPiyushReport(token, range);
    const billingMetrics = range.key === "30d"
      ? metrics
      : await fetchPiyushReport(token, billingRange);
