...
async function fetchAdityaReport(token, range) {
  ...
  params.append("campaign_ids[]", ADITYA_CAMPAIGN_ID);
...
exports.handler = async function (event) {
  if (!isAuthorized(event, "aditya")) return unauthorized();
  ...
  try {
    const metrics = await fetchAdityaReport(token, range);
    const billingMetrics = range.key === "30d"
      ? metrics
      : await fetchAdityaReport(token, billingRange);
